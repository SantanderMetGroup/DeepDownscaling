...
trainPredictGLM <- function(x,y,newdata,neighbours=1,filename) {
  ...
  predDET_reg <- pred[[2]] %>% gridArithmetics(1,operator = "+") %>% redim(drop = TRUE)
  ...
  predSTO_reg <- pred[[2]] %>% gridArithmetics(1,operator = "+") %>% redim(drop = TRUE)
...
trainDEEP <- function(x,y,architecture, epochs=10000,patience=30, 
  ...
  callbacks <- list(callback_early_stopping(patience = patience),
                    callback_model_checkpoint(filepath=paste0('./models/precip/',
                                                              architecture,'.h5'),
...
```{r experiment1, echo=TRUE, eval=TRUE, warning=FALSE, results='hide', message = FALSE}
